...
exp_2_output_log_path = os.path.join(config.TEST_DIR,'results/exp2_synth_rvtso_align_memsynth.log')
exp_2_output_result_file_path = os.path.join(config.TEST_DIR,'results/exp2_synth_rvtso_align_memsynth_result.txt')
exp_2_test_ppo_path = os.path.join(config.TEST_DIR,'results/exp2_synth_rvtso_align_memsynth_tested_ppo.txt')
align_litmus_dir = os.path.join(config.TEST_DIR,'experiment/exp2_align_memsynth')
def delete_history():
...
class TestSynth:
    ...
    def get_dif_litmus_datasets(self, src_cat, tgt_cat, src_log, tgt_log, dif_log):
        ...
        for root, dirs, files in os.walk(align_litmus_dir):
            for file in files:
        ...
        if not os.path.exists(dif_log):
            ...
            litmus_suite = sort_litmus_by_weight(dif_litmus_suite, [align_litmus_dir])
    ...
    def synth(self):
        ...
        src_log = os.path.join(config.HERD_LOG_DIR_PATH,'rvwmo-exp2-align_memsynth.log')
        tgt_log = os.path.join(config.HERD_LOG_DIR_PATH,'rvtso-exp2_align_memsynth.log')
        dif_log = os.path.join(config.HERD_LOG_DIR_PATH,'dif_litmus_rvtso-rvwmo_align_memsynth.log')
        litmus_test_suite = self.get_dif_litmus_datasets(src_cat, tgt_cat, src_log, tgt_log, dif_log)
        # print(len(litmus_test_suite))
        litmus_test_suite = [search_file(litmus, align_litmus_dir, '.litmus') for litmus in litmus_test_suite]
...
if __name__ == '__main__':
    bar = "=" * 60
    header = f"Experiment 2: synth RVTSO align memsynth"
