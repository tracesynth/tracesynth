...
exp_3_output_log_path = os.path.join(config.TEST_DIR,'results/exp3_synth_qemu.log')
exp_3_output_result_file_path = os.path.join(config.TEST_DIR,'results/exp3_synth_qemu_result.txt')
exp_3_test_ppo_path = os.path.join(config.TEST_DIR,'results/exp3_synth_qemu_tested_ppo.txt')
...
class TestQemu:
    ...
    def synth(self):
        ...
        src_log = os.path.join(config.INPUT_DIR,'herd/herd_results_rvwmo_qemu.log')
        tgt_log = os.path.join(config.INPUT_DIR,'chip_execution_logs/qemu/qemu_log.txt')
        dif_log = os.path.join(config.HERD_LOG_DIR_PATH,'dif_litmus_qemu_RVWMO.log')
...
if __name__ == '__main__':
    bar = "=" * 60
    header = f"Experiment 3: synth qemu"
